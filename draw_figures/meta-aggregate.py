       ...
       for op in rw:
           if op == 'f':
               ...
               for i, nset in enumerate(nsets):
                   ...
                   plt.ylabel("Observed Write Rate Unit: GB/s", fontsize=12)
                   plt.xlabel("Number of Datasets in a File", fontsize=12)
                   plt.rc('xtick', labelsize=11) 
                   plt.rc('ytick', labelsize=12) 
                   plt.legend([bplot["boxes"][0], bplot["boxes"][1], bplot["boxes"][2], bplot["boxes"][3]], ['default', 'non-def, ind, interleaved', 'non-def col front', 'def col front'], loc='upper left')

                   Name = "def"+ "_" + n + "_" + psize + "_" + op + ".pdf"
