   ...
   if exp == "collective":
       for psize in sizes:
           for op in rw:
               ...
               plt.ylabel("Observed Rate, Unit:GB/s", fontsize=12)
               plt.xlabel("I/O APIs Used for Write System Calls", fontsize=12)
               plt.rc('xtick', labelsize=11) 
               plt.rc('ytick', labelsize=12) 
               plt.legend([bplot["boxes"][0], bplot["boxes"][1], bplot["boxes"][2]], ['4 cores', '8 cores', '16 cores'], loc='upper right')
               if psize == '16k' and op == 'f':
                   plt.ylim(0, 0.01)
               elif psize == '256k' and op == 'f':
                   plt.ylim(0, 0.1)
               elif psize == '1024m' and op == 'f':
                   plt.ylim(0, 300)
