...
/* #endif */
        ...
        if (o->defer_flush) {
                ...
                HDF5_CHECK( H5Pget_mdc_config(accessPropList, &cache_config), "cannot get mdc config" );
                ...
                HDF5_CHECK( H5Pset_mdc_config (accessPropList, &cache_config), "cannot set mdc config" );
                if (rank == 0) {
                    fprintf(stdout,
                            "\nHDF5 defer cache flush enabled\n");
                }
        }
        ...
        char *fileName = getenv("HDF5_DATASET_SIZE_FILE"); 
        int world_rank, num_ranks, iter=0;
        char line[128];
        ...
        if (NULL != fileName) {
                useSizeFile = 1;
                all_size = (int*)malloc(num_ranks*sizeof(int));

                if (world_rank == 0) {
                        FILE* file = fopen(fileName, "r"); /* should check the result */
                        if (NULL == file) {
                                fprintf(stderr, "Error opening HDF5 dataset size file [%s], abort!\n", fileName);
                        ...
                        while (fgets(line, sizeof(line), file)) {
                                if (iter >= num_ranks)
                                        break;
                                all_size[iter] = atoi(line);
                                total_size += (hsize_t)all_size[iter];
                                iter++;
                        }
                        if (iter < num_ranks) {
                                fprintf(stderr, "Insufficient number of sizes in [%s], abort!\n", fileName);
                                MPI_Abort(MPI_COMM_WORLD, -1);
                        }
                        fclose(file);
                }
                ...
                total_size *= 1024;
                MPI_Bcast(all_size, num_ranks, MPI_INT, 0, MPI_COMM_WORLD);
                MPI_Bcast(&total_size, 1, MPI_LONG_LONG_INT, 0, MPI_COMM_WORLD);

                for (iter = 0; iter < world_rank; iter++) 
                        myStartOffset += (hsize_t) all_size[iter];

                myStartOffset *= 1024;
                int nxfer = param->blockSize / param->transferSize;
                /* fprintf(stderr, "Rank %d: original block size: %llu, transfer size: %llu\n", world_rank, param->blockSize, param->transferSize); */
                param->blockSize = all_size[world_rank] * 1024;
                param->transferSize = param->blockSize / nxfer;
                /* fprintf(stderr, "Rank %d: modified block size: %llu, transfer size: %llu\n", world_rank, param->blockSize, param->transferSize); */
                /* fprintf(stderr, "Rank %d: start off: %llu\n", world_rank, myStartOffset); */
        ...
        if (useSizeFile == 0) {
                ...
                // Tang added for reading dset size from file
                dataSetDims[0] = (hsize_t) (total_size / sizeof(IOR_size_t));
        }

        /* fprintf(stderr, "Rank %d: create dataset space size %llu\n", world_rank, dataSetDims[0]); */
                              ...
                              IOR_offset_t length, IOR_param_t * param)
{
        ...
        if (useSizeFile == 1) {
                param->offset = myStartOffset + dsetIter * param->transferSize;
                dsetIter++;
        }
        /* fprintf(stderr, "Rank %d: Seeking offset %llu\n", rank, param->offset); */
...
{
        HDF5_CHECK(H5Fflush(*(hid_t *) fd, H5F_SCOPE_GLOBAL), "cannot flush file");
